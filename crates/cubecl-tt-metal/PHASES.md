...
| `stream` | parity lane | even the new minimal single-device cross-stream regression currently hangs on TT hardware, so broader stream-facing runtime semantics are still not validated beyond focused backend-only recovery tests | keep backend-only stream recovery regressions green, keep the smaller stream helper available for future debugging, and revisit wrapper-level stream parity only after the cross-stream hang is fixed |
