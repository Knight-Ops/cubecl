...
- The live TT-local hardware-gated suite in `cubecl-tt-metal` is green end-to-end on real hardware, and `cargo test -p cubecl-tt-metal -- --list` currently reports `294` tests including manual ignored characterization hooks.
...
| `stream` | partially enabled in TT-local wrappers | the reduced and medium single-device cross-stream wrappers are green on hardware, and the TT backend now has explicit queued/submitted/completed stream state, page-sized host completion fences, binding-cursor/resource-lineage tracking for owned TT buffers, ordered queued-op batching, a CubeTask launch/source cache, TT `MeshDevice` program cache enabled at device startup, and a multi-core generic-launch partitioning path for the runtime `kernel_cube()` flow; a manual characterization showed the raw `Program` path did not increase `num_program_cache_entries()`, the full upstream-sized stream workload still timed out under a bounded 180s hardware probe, wider same-stream batching thresholds still did not clear it, and the staged full-input runtime promotion attempt regressed even `test_stream_small` and was reverted; the remaining live manual probe is `test_stream_broad_single_round_manual`, and the current blocker is broad generic-runtime throughput under the large globally-indexed stream shape rather than reduced-path stream correctness | keep `test_stream_small` and `test_stream_medium` green in isolation and in the full TT lane, keep the broad one-round stream probe manual, and continue broad stream work as a generic-runtime throughput optimization problem rather than another event-semantics bring-up |
